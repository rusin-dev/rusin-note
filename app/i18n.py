"""多语言支持：语言检测（Cookie > Accept-Language）与翻译查找

- 语言偏好通过 Cookie `rusin-lang` 保存（值: zh / en）
- 未设置时根据 Accept-Language 判断，默认中文（zh）
- 所有翻译 key 必须在 STRINGS["zh"] 与 STRINGS["en"] 中成对存在
"""
import html

from flask import Flask, g, request

LANG_COOKIE = "rusin-lang"
LANGS = ("zh", "en")
DEFAULT_LANG = "zh"

# ---------- 翻译字典 ----------
STRINGS = {
    "zh": {
        # 导航栏
        "nav_anonymous": "匿名",
        "nav_my_notes": "我的笔记",
        "nav_orgs": "我的组织",
        "nav_new_note": "新建笔记",
        "nav_share_mgmt": "分享管理",
        "nav_benben": "犇犇",
        "nav_logout": "登出",
        "nav_register": "注册",
        "nav_login": "登录",
        "nav_stats": "统计",
        "nav_disclaimer": "免责声明",
        "nav_settings": "设置",
        "nav_account_menu": "账号菜单",
        "nav_personal_space": "个人空间",
        "nav_switch_org": "切换组织",
        "nav_manage_orgs": "管理组织",
        "nav_close": "关闭",
        "lang_switch": "English",
        "theme_dark": "",
        "theme_light": "",
        # 首页
        "page_home": "首页",
        "home_tagline": "轻量级在线记事本：随手记录、即时分享、畅所欲言",
        "home_public_notes": "创建笔记",
        "home_public_notes_desc": "匿名创建公开笔记",
        "home_login": "登录",
        "home_login_desc": "登录以管理你的笔记",
        "home_register": "注册",
        "home_register_desc": "创建你的专属账号",
        "home_stats": "统计",
        "home_stats_desc": "查看全站数据统计",
        "home_benben": "犇犇",
        "home_benben_desc": "看看大家都在聊什么",
        "home_my_notes_desc": "管理你的全部私有笔记",
        "home_new_note_desc": "创建一篇新的笔记",
        "home_share_mgmt_desc": "管理你发布的分享链接",
        "home_github": "开源仓库",
        "home_github_desc": "本项目完全开源，欢迎 Star 与贡献",
        "home_recent_notes": "最近编辑的笔记",
        "home_recent_shares": "最近分享的笔记",
        # 工作台（登录后的首页）
        "wb_title": "工作台",
        "wb_welcome": "欢迎回来，{name}",
        "wb_subtitle": "随手记录，高效管理",
        "wb_new_note": "写新笔记",
        "wb_quick_actions": "快捷入口",
        "wb_overview": "数据概览",
        "wb_stat_notes": "笔记",
        "wb_stat_shares": "分享",
        "wb_stat_views": "总浏览",
        "wb_view_all": "查看全部",
        "wb_empty_notes": "还没有笔记，开始写第一篇吧",
        "wb_empty_shares": "还没有分享链接",
        "wb_todo_title": "TODO LIST",
        "wb_todo_placeholder": "添加待办事项…",
        "wb_todo_add": "添加",
        "wb_todo_empty": "暂无待办，添加一条开始吧",
        "wb_todo_toggle": "切换完成状态",
        "wb_todo_delete": "删除待办",
        "wb_todo_clear_done": "清除已完成",
        "wb_todo_err_empty": "待办内容不能为空",
        "wb_todo_err_too_long": "待办内容过长",
        "wb_todo_err_too_many": "待办数量已达上限",
        "wb_todo_err_storage": "保存失败，请稍后重试",
        # 注册
        "register_title": "注册",
        "reg_username_label": "用户名",
        "reg_password_label": "密码",
        "reg_confirm_label": "确认密码",
        "reg_submit": "注册",
        "reg_have_account": "已有账号？登录",
        "err_username_reserved": "禁止使用该用户名，请更换其他用户名",
        "err_username_invalid": "用户名只能包含字母、数字、下划线、连字符",
        "err_password_mismatch": "两次密码不一致",
        "err_csrf": "CSRF 验证失败，请刷新页面后重试",
        "err_ip_blocked": "你的 IP 已被本站封禁，无法访问",
        "err_password_weak": "密码不符合要求：{req}",
        "err_username_taken": "用户名不可用",
        # 用户设置
        "settings_title": "用户设置",
        "settings_subtitle": "当前账号：{username}",
        "settings_saved": "设置已保存",
        "settings_save": "保存",
        "settings_appearance": "界面偏好",
        "settings_simple_mode": "简洁模式",
        "settings_simple_mode_hint": "隐藏标签、置顶、犇犇等高级功能，只保留笔记读写与预览",
        "settings_simple_mode_desc": "简洁模式与账号绑定，在所有设备上一致生效。",
        "settings_password": "修改密码",
        "settings_password_desc": "修改密码会注销该账号在其它设备上的登录，当前设备保持登录。",
        "settings_password_current": "当前密码",
        "settings_password_new": "新密码（{req}）",
        "settings_password_confirm": "确认新密码",
        "settings_password_submit": "修改密码",
        "settings_username": "修改用户名",
        "settings_username_desc": "用户名是笔记、图床与附件的存储路径，修改后现有数据会自动迁移到新用户名下，旧链接将失效。",
        "settings_username_current": "当前用户名",
        "settings_username_new": "新用户名",
        "settings_username_password": "当前密码",
        "settings_username_submit": "修改用户名",
        "settings_username_confirm": "确定要修改用户名吗？修改后所有数据将迁移到新用户名。",
        "err_settings_password_wrong": "当前密码错误",
        "err_settings_username_same": "新用户名与当前用户名相同",
        "err_settings_rename_failed": "数据迁移失败，请稍后重试（原有数据未被删除）",
        "err_settings_save_failed": "保存失败，请稍后重试",
        # 登录
        "login_title": "登录",
        "login_username": "用户名",
        "login_password": "密码",
        "login_submit": "登录",
        "login_no_account": "没有账号？注册",
        "err_login_failed": "用户名或密码错误",
        # 认证
        "auth_required_title": "需要登录",
        "auth_required_body": "请先 <a href=\"/login\">登录</a> 或 <a href=\"/register\">注册</a> 以访问您的私有笔记。",
        "auth_required_body_shares": "请先 <a href=\"/login\">登录</a> 或 <a href=\"/register\">注册</a> 以访问您的分享管理。",
        "err_too_many_requests": "请求过于频繁，请稍后再试",
        # 笔记列表
        "user_notes_title": "{username} 的笔记",
        "user_new_note": "+ 新建笔记",
        "user_no_notes": "还没有笔记，创建一个吧",
        # 统计
        "stats_title": "笔记统计",
        "stats_public": "公开笔记",
        "stats_total_size": "总大小: {size}",
        "stats_private": "私有笔记",
        "stats_users": "注册用户",
        "stats_users_detail": "已注册账号",
        "stats_benben": "犇犇动态",
        "stats_benben_detail": "已发布犇犇数",
        "back_home": "返回首页",
        # 免责声明
        "disclaimer_title": "免责声明",
        "disclaimer_none": "暂无，请联系站长添加",
        "disclaimer_not_found": "免责声明文件 (Disclaimer.md) 未找到。",
        "disclaimer_read_error": "读取免责声明文件失败: {e}",
        # 笔记编辑页
        "note_public_prefix": "公开笔记",
        "note_private_prefix": "私有笔记",
        "note_share_prefix": "分享笔记",
        "note_save_btn": " 保存",
        "note_save_hint": " 按 <kbd>Ctrl</kbd> + <kbd>S</kbd> 快速保存",
        "note_last_edited": "最后编辑：",
        "note_never_edited": "尚未编辑",
        "save_status_saving": " 保存中...",
        "save_status_saved": "已保存",
        "save_status_failed": " 保存失败 ({status})",
        "save_status_net_error": " 网络错误",
        "save_hint_saved": " 已保存！按 <kbd>Ctrl</kbd> + <kbd>S</kbd> 再次保存",
        "save_hint_retry": " 保存失败，请重试",
        "save_failed_msg": " 保存失败：",
        # Markdown 只读页
        "md_readonly": "只读",
        "md_back_edit": "返回编辑",
        "md_back_share": "返回分享",
        "md_refresh": "刷新",
        "md_home": "首页",
        # 大纲预览（Markdown 只读页目录导航）
        "outline_label": "大纲",
        "outline_show": "显示大纲",
        "outline_hide": "收起大纲",
        # 分享管理
        "shares_title": "分享管理",
        "shares_create": "创建分享",
        "shares_select_note": "选择要分享的笔记",
        "shares_no_notes": "（暂无笔记，请先创建笔记）",
        "shares_editable_label": "允许编辑（访客保存将修改我的原笔记）",
        "shares_create_btn": "创建分享",
        "shares_my": "我的分享（{count}）",
        "shares_col_note": "笔记",
        "shares_col_link": "分享链接",
        "shares_col_perm": "权限",
        "shares_col_views": "查看次数",
        "shares_col_action": "操作",
        "shares_editable": "可编辑",
        "shares_readonly": "只读",
        "shares_delete": "删除",
        "shares_empty": "还没有分享链接，创建第一个吧",
        "shares_back": "返回我的笔记",
        "err_share_invalid_note": "请选择有效的笔记",
        "err_share_note_missing": "笔记不存在，请选择已有的笔记",
        "err_share_delete": "删除失败：分享不存在或无权删除",
        "err_url_invalid": "URL 不合法",
        "share_edit_hint": " 可编辑分享：保存后将写入分享者原笔记",
        # 犇犇
        "benben_title": "犇犇",
        "benben_page_info": "第 {page} 页 · 每页 {size} 条",
        "benben_label": "发布犇犇（支持 Markdown 与 LaTeX 公式，输入即预览，最多 {max} 字符）",
        "benben_submit": "发布",
        "benben_readonly": "登录后可发布犇犇，当前为只读模式",
        "benben_empty": "还没有犇犇，快来发布第一条吧",
        "benben_more": "加载更多（更早）",
        "benben_no_more": "没有更多了",
        "err_benben_empty": "内容不能为空",
        "err_benben_too_long": "内容超出长度限制（最多 {max} 字符）",
        "err_benben_cooldown": "发布过于频繁，请 {sec} 秒后再试",
        "benben_reply": "回复",
        "preview_load_failed": "预览库加载失败",
        "preview_render_failed": "预览渲染失败",
        "note_live_preview": "实时渲染",
        "note_live_preview_hint": "实时渲染已关闭，点击此处开启",
        "md_manual": "Markdown 使用手册",
        "note_editor_label": "编辑器",
        "note_preview_label": "预览",
        "note_splitter_label": "拖动调整编辑器与预览宽度（双击恢复默认）",
        "preview_show": "预览",
        "preview_edit": "编辑",
        "note_refs_label": "引用笔记",
        "note_refs_recent": "最近编辑",
        "note_refs_no_match": "没有匹配的笔记",
        # 笔记标签
        "note_tags_label": "标签",
        "note_tags_placeholder": "添加标签，逗号分隔",
        "note_tags_all": "全部",
        "note_tags_no_match": "该标签下没有笔记",
        # 笔记文件夹
        "note_folders_label": "文件夹",
        "note_folders_placeholder": "文件夹路径，用 / 分层，如 工作/项目A，留空不归类",
        "note_folders_no_match": "该文件夹下没有笔记",
        "note_folders_uncategorized": "未归类",
        "note_folders_toggle": "展开 / 折叠",
        "note_folders_filter": "只看该文件夹及其子文件夹",
        # 笔记置顶
        "note_pins_toggle": "置顶 / 取消置顶",
        # 笔记删除
        "note_delete": "删除笔记",
        "note_delete_confirm": "确定删除这篇笔记吗？此操作不可撤销。",
        # Markdown 标题锚点
        "note_anchor_title": "跳转到此标题",
        # Markdown 提示卡片（[!NOTE] 等 GitHub 风格可折叠卡片）
        "md_alert_note": "说明",
        "md_alert_tip": "提示",
        "md_alert_important": "重要",
        "md_alert_warning": "警告",
        "md_alert_caution": "注意",
        # 笔记图床
        "feature_note_images": "笔记图床",
        "user_images_link": "图床管理",
        "images_title": "图床管理",
        "images_usage": "已用 {used} / {total}（{count} 张）",
        "images_empty": "还没有上传图片，在笔记编辑器里粘贴或拖入图片即可",
        "images_insert_hint": "在笔记中引用：![图片]({url})",
        "images_delete": "删除",
        "images_delete_confirm": "确定删除这张图片？引用它的笔记将无法显示",
        "images_back": "返回我的笔记",
        "err_image_too_large": "图片超出大小限制（最多 {max} KB）",
        "err_file_too_large": "文件超出大小限制（最多 {max} KB）",
        "err_image_format": "仅支持 PNG / JPEG / GIF / WebP 格式的图片",
        "err_image_quota": "图床空间不足（配额 {total}），请先删除旧图片",
        "err_image_upload": "上传失败，请重试",
        "note_images_uploading": "图片上传中…",
        "note_images_done": "图片已插入",
        "note_images_failed": "图片上传失败",
        # 笔记附件
        "feature_note_attachments": "笔记附件",
        "user_attachments_link": "附件管理",
        "attachments_title": "附件管理",
        "attachments_usage": "已用 {used} / {total}（{count} 个）",
        "attachments_empty": "还没有上传附件，在笔记编辑器里点击附件按钮上传",
        "attachments_insert_hint": "在笔记中引用：[文件名]({url})",
        "attachments_upload_hint": "点击或拖拽文件到此处上传",
        "attachments_upload_limit": "单个文件最大 {max} KB",
        "attachments_delete": "删除",
        "attachments_delete_confirm": "确定删除这个附件？引用它的笔记将无法下载",
        "attachments_back": "返回我的笔记",
        "attachments_download_login_hint": "附件仅登录用户可下载（匿名访客会被拒绝）",
        "err_attachment_too_large": "附件超出大小限制（最多 {max} KB）",
        "err_attachment_blocked_type": "不支持的文件类型（可执行文件被禁止）",
        "err_attachment_no_filename": "文件名无效",
        "err_attachment_no_file": "请选择要上传的文件",
        "err_attachment_empty": "文件为空",
        "err_attachment_quota": "附件空间不足（配额 {total}），请先删除旧附件",
        "err_attachment_note_quota": "该笔记的附件总大小超过 {total} KB，请删除部分附件或改用更小的文件",
        "err_attachment_upload": "上传失败，请重试",
        "err_attachment_download_login_required": "请先登录后再下载附件",
        "err_attachment_download_busy": "同时下载的附件过多（上限 {max} 个），请等待当前下载完成后重试",
        "err_attachment_upload_busy": "同时上传的附件过多（上限 {max} 个），请等待当前上传完成后重试",
        "note_attachment_btn": "附件",
        "note_attachments_uploading": "附件上传中…",
        "note_attachments_done": "附件已插入",
        "note_attachments_failed": "附件上传失败",
        # 评论系统
        "feature_comments": "评论系统",
        "comments_title": "评论",
        "comments_count": "共 {count} 条评论",
        "comments_label": "发表评论（最多 {max} 字）",
        "comments_submit": "发表评论",
        "comments_anonymous": "匿名评论",
        "comments_logged_in_as": "以身份 ",
        "comments_empty": "还没有评论，快来发表第一条评论吧",
        "comments_more": "加载更多",
        "comments_no_more": "没有更多评论了",
        "comments_back": "返回",
        "comments_login_hint": "请登录后发表评论",
        "err_comment_empty": "评论内容不能为空",
        "err_comment_too_long": "评论内容超出长度限制（最多 {max} 字）",
        "err_comment_cooldown": "评论太频繁，请 {sec} 秒后再试",
        "err_comment_upload": "评论发布失败，请重试",
        # 功能开关（#90）
        "feature_world_notes": "公开笔记",
        "feature_benben": "犇犇动态",
        "feature_share_links": "分享链接",
        "feature_open_register": "开放注册",
        "feature_note_refs": "笔记快捷引用",
        "feature_note_tags": "笔记标签",
        "feature_note_folders": "笔记文件夹",
        "feature_note_pins": "笔记置顶",
        "feature_note_attachments": "笔记附件",
        "feature_heading_anchors": "Markdown 标题锚点",
        "feature_markdown_alerts": "Markdown 提示卡片",
        "feature_latex_render": "LaTeX 公式渲染",
        "feature_code_highlight": "代码高亮",
        "feature_avatar": "用户头像",
        "features_status_title": "功能状态",
        "features_status_desc": "站点当前启用 / 停用的功能（由管理员配置）",
        "feature_on": "已启用",
        "feature_off": "已停用",
        "admin_features_title": "功能开关管理",
        "admin_features_desc": "通过滑块开关启用或停用站点功能，保存后立即生效，启用中的功能会在统计页呈现",
        "admin_features_submit": "保存设置",
        "admin_features_saved": "设置已保存",
        "admin_features_link": "管理功能开关",
        # 组织
        "feature_orgs": "组织功能",
        "org_owner": "创建者",
        "org_join_policy_invite": "邀请制",
        "org_join_policy_public": "公开加入",
        "org_join_policy_approve": "审批加入",
        "org_join_public": "公开加入",
        "org_request_join": "申请加入",
        "org_invite_only": "此组织仅可通过邀请加入",
        "org_join_message_placeholder": "申请留言（可选）",
        "org_notes": "组织笔记",
        "org_members": "成员管理",
        "org_settings": "组织设置",
        "org_invites": "邀请管理",
        "org_requests": "加入申请",
        "org_back": "返回组织",
        "org_members_title": "成员列表",
        "org_members_empty": "暂无成员",
        "org_member_user": "用户",
        "org_member_role": "角色",
        "org_member_joined": "加入时间",
        "org_member_actions": "操作",
        "org_role_owner": "所有者",
        "org_role_admin": "管理员",
        "org_role_member": "成员",
        "org_note_new": "新建笔记",
        "org_notes_empty": "暂无笔记",
        "org_note_view": "查看",
        "org_note_edit": "编辑",
        "org_note_delete": "删除",
        "org_back_mine": "返回我的组织",
        "org_create_title": "创建组织",
        "org_create_org_name": "组织 ID",
        "org_create_org_name_placeholder": "英文、数字、下划线、连字符",
        "org_create_org_name_hint": "唯一标识，不可更改",
        "org_create_name": "显示名称",
        "org_create_name_placeholder": "例如：我的团队",
        "org_create_description": "描述",
        "org_create_description_placeholder": "简要描述组织用途",
        "org_create_join_policy": "加入方式",
        "org_create_submit": "创建组织",
        "org_mine_title": "我的组织",
        "org_mine_empty": "你还未加入任何组织",
        "org_mine_name": "组织 ID",
        "org_mine_display_name": "名称",
        "org_mine_role": "我的角色",
        "org_mine_actions": "操作",
        "org_view": "查看",
        "org_leave": "退出",
        "org_settings_basic": "基本信息",
        "org_settings_name": "显示名称",
        "org_settings_description": "描述",
        "org_settings_join_policy": "加入方式",
        "org_settings_save": "保存设置",
        "org_settings_members": "成员管理",
        "org_settings_danger": "危险操作",
        "org_remove": "移除",
        "org_delete": "删除组织",
        "org_invites_title": "邀请管理",
        "org_invite_created": "邀请码已创建",
        "org_invite_share_hint": "分享此邀请码给需要加入的用户",
        "org_invite_create": "创建邀请码",
        "org_invite_type": "邀请类型",
        "org_invite_type_invite": "直接邀请",
        "org_invite_type_approve": "审批邀请",
        "org_invite_expires": "有效期",
        "org_invite_days": "天",
        "org_invite_generate": "生成邀请码",
        "org_invites_active": "有效邀请码",
        "org_invite_code": "邀请码",
        "org_invite_created_by": "创建者",
        "org_invite_expires_at": "过期时间",
        "org_invite_actions": "操作",
        "org_invite_revoke": "撤销",
        "org_invites_empty": "暂无有效邀请码",
        "org_requests_title": "加入申请",
        "org_request_user": "申请人",
        "org_request_message": "留言",
        "org_request_time": "申请时间",
        "org_request_status": "状态",
        "org_request_status_pending": "待审批",
        "org_request_status_approved": "已通过",
        "org_request_status_rejected": "已拒绝",
        "org_request_approve": "通过",
        "org_request_reject": "拒绝",
        "org_requests_empty": "暂无加入申请",
        "confirm_remove_member": "确定要移除该成员吗？",
        "confirm_delete_org": "确定要删除该组织吗？此操作不可恢复！",
        "confirm_leave_org": "确定要退出该组织吗？",
        "confirm_delete": "确定要删除吗？",
        "note_id": "笔记 ID",
        "note_mtime": "修改时间",
        "note_size": "大小",
        "note_actions": "操作",
        "note_save": "保存",
        "note_content_placeholder": "使用 Markdown 编写笔记内容...",
    },
    "en": {
        # Navbar
        "nav_anonymous": "Anonymous",
        "nav_my_notes": "My Notes",
        "nav_orgs": "My Orgs",
        "nav_new_note": "New Note",
        "nav_share_mgmt": "Shares",
        "nav_benben": "Benben",
        "nav_logout": "Logout",
        "nav_register": "Register",
        "nav_login": "Log In",
        "nav_stats": "Stats",
        "nav_disclaimer": "Disclaimer",
        "nav_settings": "Settings",
        "nav_account_menu": "Account menu",
        "nav_personal_space": "Personal Space",
        "nav_switch_org": "Switch organization",
        "nav_manage_orgs": "Manage Orgs",
        "nav_close": "Close",
        "lang_switch": "简体中文",
        "theme_dark": "",
        "theme_light": "",
        # Home
        "page_home": "Home",
        "home_tagline": "A lightweight online notepad: jot it down, share instantly, speak freely",
        "home_public_notes": "Public Note (Anonymous)",
        "home_public_notes_desc": "Browse public notes anonymously",
        "home_register": "Sign Up",
        "home_register_desc": "Create your own account",
        "home_login": "Log In",
        "home_login_desc": "Log in to manage your notes",
        "home_stats": "Stats",
        "home_stats_desc": "View site-wide statistics",
        "home_disclaimer": "Disclaimer",
        "home_disclaimer_desc": "Read the terms and disclaimer",
        "home_benben": "Benben",
        "home_benben_desc": "See what everyone is talking about",
        "home_my_notes_desc": "Manage all your private notes",
        "home_new_note_desc": "Create a new note",
        "home_share_mgmt_desc": "Manage the share links you publish",
        "home_github": "Open Source",
        "home_github_desc": "Fully open source. Star it and contribute!",
        "home_recent_notes": "Recently Edited Notes",
        "home_recent_shares": "Recent Shares",
        # Workbench (homepage after login)
        "wb_title": "Workspace",
        "wb_welcome": "Welcome back, {name}",
        "wb_subtitle": "Capture ideas and manage them efficiently",
        "wb_new_note": "New Note",
        "wb_quick_actions": "Quick actions",
        "wb_overview": "Overview",
        "wb_stat_notes": "Notes",
        "wb_stat_shares": "Shares",
        "wb_stat_views": "Total views",
        "wb_view_all": "View all",
        "wb_empty_notes": "No notes yet — write your first one",
        "wb_empty_shares": "No share links yet",
        "wb_todo_title": "TODO LIST",
        "wb_todo_placeholder": "Add a task…",
        "wb_todo_add": "Add",
        "wb_todo_empty": "No tasks yet — add one to get started",
        "wb_todo_toggle": "Toggle done",
        "wb_todo_delete": "Delete task",
        "wb_todo_clear_done": "Clear completed",
        "wb_todo_err_empty": "Task cannot be empty",
        "wb_todo_err_too_long": "Task is too long",
        "wb_todo_err_too_many": "You have reached the task limit",
        "wb_todo_err_storage": "Failed to save, please retry",
        # Register
        "register_title": "Register",
        "reg_username_label": "Username",
        "reg_password_label": "Password",
        "reg_confirm_label": "Confirm Password",
        "reg_submit": "Register",
        "reg_have_account": "Already have an account? Log in",
        "err_username_reserved": "This username is reserved, please choose another",
        "err_username_invalid": "Username may only contain letters, digits, underscores and hyphens",
        "err_password_mismatch": "Passwords do not match",
        "err_csrf": "CSRF verification failed, please refresh and try again",
        "err_ip_blocked": "Your IP has been blocked from this site",
        "err_password_weak": "Password does not meet requirements: {req}",
        "err_username_taken": "Username unavailable",
        # User settings
        "settings_title": "User Settings",
        "settings_subtitle": "Signed in as {username}",
        "settings_saved": "Settings saved",
        "settings_save": "Save",
        "settings_appearance": "Appearance",
        "settings_simple_mode": "Simple mode",
        "settings_simple_mode_hint": "Hide tags, pins, benben and other advanced features; keep only note editing and preview",
        "settings_simple_mode_desc": "Simple mode is tied to your account and applies on all devices.",
        "settings_password": "Change Password",
        "settings_password_desc": "Changing your password logs you out on other devices; this device stays signed in.",
        "settings_password_current": "Current password",
        "settings_password_new": "New password ({req})",
        "settings_password_confirm": "Confirm new password",
        "settings_password_submit": "Change password",
        "settings_username": "Change Username",
        "settings_username_desc": "Your username is the storage path for notes, images and attachments. Changing it migrates existing data to the new username; old links stop working.",
        "settings_username_current": "Current username",
        "settings_username_new": "New username",
        "settings_username_password": "Current password",
        "settings_username_submit": "Change username",
        "settings_username_confirm": "Change your username? All data will be migrated to the new username.",
        "err_settings_password_wrong": "Current password is incorrect",
        "err_settings_username_same": "New username is the same as the current one",
        "err_settings_rename_failed": "Data migration failed, please try again later (existing data was not deleted)",
        "err_settings_save_failed": "Failed to save, please try again later",
        # Login
        "login_title": "Log In",
        "login_username": "Username",
        "login_password": "Password",
        "login_submit": "Log In",
        "login_no_account": "No account? Register",
        "err_login_failed": "Invalid username or password",
        # Auth
        "auth_required_title": "Login Required",
        "auth_required_body": "Please <a href=\"/login\">log in</a> or <a href=\"/register\">register</a> to access your private notes.",
        "auth_required_body_shares": "Please <a href=\"/login\">log in</a> or <a href=\"/register\">register</a> to access your share management.",
        "err_too_many_requests": "Too many requests, please try again later",
        # Note list
        "user_notes_title": "{username}'s Notes",
        "user_new_note": "+ New Note",
        "user_no_notes": "No notes yet, create one",
        # Stats
        "stats_title": "Note Statistics",
        "stats_public": "Public Notes",
        "stats_total_size": "Total size: {size}",
        "stats_private": "Private Notes",
        "stats_users": "Registered Users",
        "stats_users_detail": "Registered accounts",
        "stats_benben": "Benben Posts",
        "stats_benben_detail": "Total posts published",
        "back_home": "Back to Home",
        # Disclaimer
        "disclaimer_title": "Disclaimer",
        "disclaimer_none": "None yet, contact the admin",
        "disclaimer_not_found": "Disclaimer file (Disclaimer.md) not found.",
        "disclaimer_read_error": "Failed to read disclaimer file: {e}",
        # Note editor
        "note_public_prefix": "Public Note",
        "note_private_prefix": "Private Note",
        "note_share_prefix": "Shared Note",
        "note_save_btn": " Save",
        "note_save_hint": " Press <kbd>Ctrl</kbd> + <kbd>S</kbd> to save",
        "note_last_edited": "Last edited: ",
        "note_never_edited": "Not edited yet",
        "save_status_saving": " Saving...",
        "save_status_saved": "Saved",
        "save_status_failed": " Save failed ({status})",
        "save_status_net_error": " Network error",
        "save_hint_saved": " Saved! Press <kbd>Ctrl</kbd> + <kbd>S</kbd> to save again",
        "save_hint_retry": " Save failed, please retry",
        "save_failed_msg": " Save failed: ",
        # Markdown read-only
        "md_readonly": "Read-only",
        "md_back_edit": "Back to edit",
        "md_back_share": "Back to share",
        "md_refresh": "Refresh",
        "md_home": "Home",
        # Outline preview (TOC navigation on the Markdown read-only page)
        "outline_label": "Outline",
        "outline_show": "Show outline",
        "outline_hide": "Hide outline",
        # Share management
        "shares_title": "Share Management",
        "shares_create": "Create Share",
        "shares_select_note": "Select a note to share",
        "shares_no_notes": "(No notes yet, create one first)",
        "shares_editable_label": "Allow editing (visitor saves will modify my original note)",
        "shares_create_btn": "Create Share",
        "shares_my": "My Shares ({count})",
        "shares_col_note": "Note",
        "shares_col_link": "Share Link",
        "shares_col_perm": "Permission",
        "shares_col_views": "Views",
        "shares_col_action": "Action",
        "shares_editable": "Editable",
        "shares_readonly": "Read-only",
        "shares_delete": "Delete",
        "shares_empty": "No shares yet, create one",
        "shares_back": "Back to My Notes",
        "err_share_invalid_note": "Please select a valid note",
        "err_share_note_missing": "Note does not exist, please select an existing one",
        "err_share_delete": "Delete failed: share not found or not yours",
        "err_url_invalid": "Invalid URL",
        "share_edit_hint": " Editable share: saves will be written back to the owner's note",
        # Benben
        "benben_title": "Benben",
        "benben_page_info": "Page {page} · {size} per page",
        "benben_label": "Post to Benben (Markdown & LaTeX supported, live preview, max {max} chars)",
        "benben_submit": "Post",
        "benben_readonly": "Log in to post; currently read-only",
        "benben_empty": "No posts yet, be the first!",
        "benben_more": "Load more (earlier)",
        "benben_no_more": "No more",
        "err_benben_empty": "Content cannot be empty",
        "err_benben_too_long": "Content exceeds the length limit (max {max} chars)",
        "err_benben_cooldown": "Posting too frequently, try again in {sec} seconds",
        "benben_reply": "Reply",
        "preview_load_failed": "Preview library failed to load",
        "preview_render_failed": "Preview rendering failed",
        "note_live_preview": "Live Render",
        "note_live_preview_hint": "Live preview is off · click to enable",
        "md_manual": "Markdown Guide",
        "note_editor_label": "Editor",
        "note_preview_label": "Preview",
        "note_splitter_label": "Drag to resize editor and preview (double-click to reset)",
        "preview_show": "Preview",
        "preview_edit": "Edit",
        "note_refs_label": "Reference a note",
        "note_refs_recent": "Recently edited",
        "note_refs_no_match": "No matching notes",
        # Note tags
        "note_tags_label": "Tags",
        "note_tags_placeholder": "Add tags, separated by commas",
        "note_tags_all": "All",
        "note_tags_no_match": "No notes with this tag",
        # Note folders
        "note_folders_label": "Folder",
        "note_folders_placeholder": "Folder path, use / for subfolders, e.g. work/projectA, empty for none",
        "note_folders_no_match": "No notes in this folder",
        "note_folders_uncategorized": "Uncategorized",
        "note_folders_toggle": "Expand / collapse",
        "note_folders_filter": "Only this folder and its subfolders",
        # Note pinning
        "note_pins_toggle": "Pin / unpin",
        "note_delete": "Delete note",
        "note_delete_confirm": "Delete this note? This cannot be undone.",
        # Markdown heading anchors
        "note_anchor_title": "Link to this heading",
        # Markdown alert cards ([!NOTE] etc., collapsible)
        "md_alert_note": "Note",
        "md_alert_tip": "Tip",
        "md_alert_important": "Important",
        "md_alert_warning": "Warning",
        "md_alert_caution": "Caution",
        # Note image hosting
        "feature_note_images": "Note Image Hosting",
        "user_images_link": "Image Hosting",
        "images_title": "Image Hosting",
        "images_usage": "Used {used} of {total} ({count} images)",
        "images_empty": "No images yet — paste or drop an image in the note editor",
        "images_insert_hint": "Reference in a note: ![image]({url})",
        "images_delete": "Delete",
        "images_delete_confirm": "Delete this image? Notes referencing it will lose it",
        "images_back": "Back to My Notes",
        "err_image_too_large": "Image exceeds the size limit (max {max} KB)",
        "err_file_too_large": "File exceeds the size limit (max {max} KB)",
        "err_image_format": "Only PNG / JPEG / GIF / WebP images are supported",
        "err_image_quota": "Image hosting quota exceeded ({total}); delete old images first",
        "err_image_upload": "Upload failed, please retry",
        "note_images_uploading": "Uploading image…",
        "note_images_done": "Image inserted",
        "note_images_failed": "Image upload failed",
        # Note attachments
        "feature_note_attachments": "Note Attachments",
        "user_attachments_link": "Attachments",
        "attachments_title": "Attachment Management",
        "attachments_usage": "Used {used} of {total} ({count} attachments)",
        "attachments_empty": "No attachments yet — click the attachment button in the note editor to upload",
        "attachments_insert_hint": "Reference in a note: [filename]({url})",
        "attachments_upload_hint": "Click or drag files here to upload",
        "attachments_upload_limit": "Max file size: {max} KB",
        "attachments_delete": "Delete",
        "attachments_delete_confirm": "Delete this attachment? Notes referencing it will lose it",
        "attachments_back": "Back to My Notes",
        "attachments_download_login_hint": "Attachments can only be downloaded by logged-in users (anonymous access is rejected)",
        "err_attachment_too_large": "Attachment exceeds the size limit (max {max} KB)",
        "err_attachment_blocked_type": "Unsupported file type (executable files are blocked)",
        "err_attachment_no_filename": "Invalid filename",
        "err_attachment_no_file": "Please select a file to upload",
        "err_attachment_empty": "Empty file",
        "err_attachment_quota": "Attachment quota exceeded ({total}); delete old attachments first",
        "err_attachment_note_quota": "This note's attachments exceed {total} KB; remove some attachments or use smaller files",
        "err_attachment_upload": "Upload failed, please retry",
        "err_attachment_download_login_required": "Please log in before downloading attachments",
        "err_attachment_download_busy": "Too many concurrent attachment downloads (limit {max}); wait for the current download to finish and retry",
        "err_attachment_upload_busy": "Too many concurrent attachment uploads (limit {max}); wait for the current upload to finish and retry",
        "note_attachment_btn": "Attachment",
        "note_attachments_uploading": "Uploading attachment…",
        "note_attachments_done": "Attachment inserted",
        "note_attachments_failed": "Attachment upload failed",
        # Comments
        "feature_comments": "Comments",
        "comments_title": "Comments",
        "comments_count": "{count} comments",
        "comments_label": "Post a comment (max {max} characters)",
        "comments_submit": "Post Comment",
        "comments_anonymous": "Post anonymously",
        "comments_logged_in_as": "Posting as ",
        "comments_empty": "No comments yet — be the first to comment",
        "comments_more": "Load more",
        "comments_no_more": "No more comments",
        "comments_back": "Go Back",
        "comments_login_hint": "Please log in to post a comment",
        "err_comment_empty": "Comment cannot be empty",
        "err_comment_too_long": "Comment exceeds the length limit (max {max} characters)",
        "err_comment_cooldown": "Posting too frequently, please try again in {sec} seconds",
        "err_comment_upload": "Failed to post comment, please retry",
        # Feature flags (#90)
        "feature_world_notes": "Public Notes",
        "feature_benben": "Benben Feed",
        "feature_share_links": "Share Links",
        "feature_open_register": "Open Registration",
        "feature_note_refs": "Note Quick References",
        "feature_note_tags": "Note Tags",
        "feature_note_folders": "Note Folders",
        "feature_note_pins": "Note Pinning",
        "feature_note_attachments": "Note Attachments",
        "feature_heading_anchors": "Markdown Heading Anchors",
        "feature_markdown_alerts": "Markdown Alerts",
        "feature_latex_render": "LaTeX Rendering",
        "feature_code_highlight": "Code Highlighting",
        "feature_avatar": "User Avatars",
        "features_status_title": "Feature Status",
        "features_status_desc": "Features currently enabled on this site (configured by the admin)",
        "feature_on": "Enabled",
        "feature_off": "Disabled",
        "admin_features_title": "Feature Flags",
        "admin_features_desc": "Toggle site features on or off with the switches. Changes take effect immediately; enabled features are shown on the stats page",
        "admin_features_submit": "Save",
        "admin_features_saved": "Settings saved",
        "admin_features_link": "Manage feature flags",
        # Orgs
        "feature_orgs": "Organizations",
        "org_owner": "Owner",
        "org_join_policy_invite": "Invite only",
        "org_join_policy_public": "Public",
        "org_join_policy_approve": "Approval required",
        "org_join_public": "Join Publicly",
        "org_request_join": "Request to Join",
        "org_invite_only": "This organization is invite-only",
        "org_join_message_placeholder": "Message (optional)",
        "org_notes": "Org Notes",
        "org_members": "Members",
        "org_settings": "Settings",
        "org_invites": "Invites",
        "org_requests": "Requests",
        "org_back": "Back to Org",
        "org_members_title": "Members",
        "org_members_empty": "No members yet",
        "org_member_user": "User",
        "org_member_role": "Role",
        "org_member_joined": "Joined",
        "org_member_actions": "Actions",
        "org_role_owner": "Owner",
        "org_role_admin": "Admin",
        "org_role_member": "Member",
        "org_note_new": "New Note",
        "org_notes_empty": "No notes yet",
        "org_note_view": "View",
        "org_note_edit": "Edit",
        "org_note_delete": "Delete",
        "org_back_mine": "Back to My Orgs",
        "org_create_title": "Create Organization",
        "org_create_org_name": "Organization ID",
        "org_create_org_name_placeholder": "Letters, numbers, underscore, hyphen",
        "org_create_org_name_hint": "Unique identifier, cannot be changed",
        "org_create_name": "Display Name",
        "org_create_name_placeholder": "e.g. My Team",
        "org_create_description": "Description",
        "org_create_description_placeholder": "Briefly describe this organization",
        "org_create_join_policy": "Join Policy",
        "org_create_submit": "Create Organization",
        "org_mine_title": "My Organizations",
        "org_mine_empty": "You haven't joined any organizations yet",
        "org_mine_name": "Org ID",
        "org_mine_display_name": "Name",
        "org_mine_role": "My Role",
        "org_mine_actions": "Actions",
        "org_view": "View",
        "org_leave": "Leave",
        "org_settings_basic": "Basic Info",
        "org_settings_name": "Display Name",
        "org_settings_description": "Description",
        "org_settings_join_policy": "Join Policy",
        "org_settings_save": "Save Settings",
        "org_settings_members": "Member Management",
        "org_settings_danger": "Danger Zone",
        "org_remove": "Remove",
        "org_delete": "Delete Organization",
        "org_invites_title": "Invite Management",
        "org_invite_created": "Invite code created",
        "org_invite_share_hint": "Share this code with users who want to join",
        "org_invite_create": "Create Invite Code",
        "org_invite_type": "Invite Type",
        "org_invite_type_invite": "Direct Invite",
        "org_invite_type_approve": "Approval Invite",
        "org_invite_expires": "Expires in",
        "org_invite_days": "days",
        "org_invite_generate": "Generate Code",
        "org_invites_active": "Active Codes",
        "org_invite_code": "Code",
        "org_invite_created_by": "Created by",
        "org_invite_expires_at": "Expires at",
        "org_invite_actions": "Actions",
        "org_invite_revoke": "Revoke",
        "org_invites_empty": "No active invite codes",
        "org_requests_title": "Join Requests",
        "org_request_user": "Applicant",
        "org_request_message": "Message",
        "org_request_time": "Time",
        "org_request_status": "Status",
        "org_request_status_pending": "Pending",
        "org_request_status_approved": "Approved",
        "org_request_status_rejected": "Rejected",
        "org_request_approve": "Approve",
        "org_request_reject": "Reject",
        "org_requests_empty": "No join requests",
        "confirm_remove_member": "Are you sure you want to remove this member?",
        "confirm_delete_org": "Are you sure you want to delete this organization? This cannot be undone!",
        "confirm_leave_org": "Are you sure you want to leave this organization?",
        "confirm_delete": "Are you sure you want to delete this?",
        "note_id": "Note ID",
        "note_mtime": "Modified",
        "note_size": "Size",
        "note_actions": "Actions",
        "note_save": "Save",
        "note_content_placeholder": "Write your note in Markdown...",
    },
}


def detect_lang(handler) -> str:
    """兼容旧 BaseHTTPRequestHandler 接口（保留供其他模块调用）"""
    cookie = handler.headers.get("Cookie", "")
    for pair in cookie.split(";"):
        pair = pair.strip()
        if pair.startswith("rusin-lang="):
            value = pair[len("rusin-lang="):].strip()
            if value in LANGS:
                return value
    accept = handler.headers.get("Accept-Language", "")
    first = accept.split(",")[0].strip().lower()
    if first.startswith("zh"):
        return "zh"
    if first.startswith("en"):
        return "en"
    return DEFAULT_LANG


def detect_lang_from_request() -> str:
    """Flask 请求上下文下的语言检测（Cookie > Accept-Language > 默认中文）"""
    cookie = request.headers.get("Cookie", "")
    for pair in cookie.split(";"):
        pair = pair.strip()
        if pair.startswith(f"{LANG_COOKIE}="):
            value = pair[len(LANG_COOKIE) + 1:].strip()
            if value in LANGS:
                return value
    accept = request.headers.get("Accept-Language", "")
    first = accept.split(",")[0].strip().lower()
    if first.startswith("zh"):
        return "zh"
    if first.startswith("en"):
        return "en"
    return DEFAULT_LANG


def register_i18n(app: Flask) -> None:
    """注册 Jinja2 全局上下文，使模板可直接用 {{ t('key') }} / {{ lang }} / {{ theme }} / {{ theme_script }} / {{ theme_vars }} / {{ current_user }} / {{ site_name }}"""
    from . import config as _cfg
    from .feature_flags import feature_enabled
    from .theme import THEME_VARS, get_theme_script
    from .utils import (
        get_avatar_url, render_code_highlight_head, render_heading_anchors_head,
        render_markdown_alerts_head, render_pygments_head,
    )

    @app.context_processor
    def inject_globals():
        lang = getattr(g, "lang", DEFAULT_LANG)
        current_user = getattr(g, "current_user", None)
        # 导航栏用户下拉框的「切换组织」列表：仅登录且启用组织功能时构建，
        # 供各页面直接渲染（页面缓存按访问者隔离，键见 views/_helpers.page_cache_key）
        user_orgs = []
        if current_user and feature_enabled("orgs"):
            from .store import get_org, get_user_orgs
            for org_name in get_user_orgs(current_user):
                org = get_org(org_name) or {}
                user_orgs.append({"id": org_name, "name": org.get("name") or org_name})
        return {
            "t": lambda key, **kw: t(lang, key, **kw),
            "lang": lang,
            "lang_switch_url": "/lang/" + ("en" if lang == "zh" else "zh"),
            "theme": getattr(g, "theme", None),
            "theme_script": get_theme_script(lang),
            "theme_vars": THEME_VARS,
            "simple_mode": getattr(g, "simple_mode", False),
            "pygments_head": render_pygments_head(),
            "current_user": current_user,
            "user_orgs": user_orgs,
            "site_name": _cfg.SITE_NAME,
            "code_highlight_head": render_code_highlight_head(),
            "heading_anchors_head": render_heading_anchors_head(),
            "markdown_alerts_head": render_markdown_alerts_head(),
            "get_avatar": get_avatar_url,
            "feature_enabled": feature_enabled,
        }


def t(lang: str, key: str, **fmt) -> str:
    """按语言取翻译；key 缺失时返回 key 本身（便于发现遗漏）"""
    text = STRINGS.get(lang, {}).get(key)
    if text is None:
        text = STRINGS.get(DEFAULT_LANG, {}).get(key, key)
    if fmt:
        try:
            return text.format(**fmt)
        except (KeyError, IndexError):
            return text
    return text


def get_lang_switch(lang: str) -> str:
    """导航栏语言切换链接（切换到另一种语言）"""
    target = "en" if lang == "zh" else "zh"
    return f'<a href="/lang/{target}"><i class="fa-solid fa-language" aria-hidden="true"></i>{html.escape(t(lang, "lang_switch"))}</a>'


def get_theme_labels_js(lang: str) -> str:
    """主题按钮文字映射（注入 THEME_SCRIPT 用）"""
    import json
    return json.dumps({
        "dark": t(lang, "theme_dark"),
        "light": t(lang, "theme_light"),
    }, ensure_ascii=False)
