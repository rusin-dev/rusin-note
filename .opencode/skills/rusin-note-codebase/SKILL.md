...
- 生产建议（Linux，需另装 gunicorn）：`gunicorn 'app.wsgi:app' -b 0.0.0.0:${PORT:-8080} --workers 2 --threads 4`
...
- 数据目录：`RUSIN_DATA_DIR` 环境变量（默认 `data`，即项目下 `data/`，JSON 内容 + SQLite 索引均在此，本地 sqlite/file 后端使用；日志 `log/` 与插件 `plugins/` 目录始终在该目录下、与后端无关）；Zeabur 等平台挂卷到 `/data` 并设 `RUSIN_DATA_DIR=/data`
...
| `NOTICE.txt` | 首页公告横幅内容（取第一个非空行，见 `utils.read_notice_first_line`） |
...
前端**无独立 JS/CSS 文件**（无 `static/`），脚本/样式全部内联在模板的 `<script>`/`<style>` 中；`partials/_card_theme_css.html`、`_navbar_css.html`、`_page_transition_css.html` 是纯 CSS 片段，被 `<style>{% include %}</style>` 引入。改动模板后请运行 `python tests/frontend_check.py`（Jinja2 语法 + 内联 CSS 配平 + JSON；本机有 Node 时才额外做内联 JS `--check`，否则静默跳过）。
